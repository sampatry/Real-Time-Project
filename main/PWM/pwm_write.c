...
void PWM_output_update(void* pvParameters) {
    ...
    while (1){
        ...
        if (xQueueReceive(config->QUEUE, &pulse_width_out_us, portMAX_DELAY) == pdPASS){
            ESP_LOGI(TAG_PWM_LEDC, "The pulse width is: %d and the period is: %d", pulse_width_out_us, (1000000/config->FREQ_HZ));
...
esp_err_t PWM_output_config(motor_config_t *config) {
    ...
    if (ledc_timer_config(&timer_conf) != ESP_OK) { //  Apply timer config
        ESP_LOGE(TAG_PWM_LEDC, "Failed to configure PWM output timer for gpio: %d", config->GPIO);
        return ESP_FAIL;
    }
    ...
    ledc_channel_config_t ledc_conf = {
     ...
     if (ledc_channel_config(&ledc_conf) != ESP_OK) { //  Apply channel config
        ESP_LOGE(TAG_PWM_LEDC, "Failed to configure PWM output channel for gpio: %d", config->GPIO);
        return ESP_FAIL;
    }
